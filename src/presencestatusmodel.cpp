...
{
   Delegates::getPresenceSerializationDelegate()->serialize();
}
...
{
    return Delegates::getPresenceSerializationDelegate()->isTracked(backend);
}

void PresenceStatusModel::setAutoTracked(CollectionInterface* backend, bool tracked) const
{
    Delegates::getPresenceSerializationDelegate()->setTracked(backend,tracked);
}
