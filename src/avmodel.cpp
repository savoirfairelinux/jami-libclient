...
AVModelPimpl::startedDecoding(const QString& id, const QString& shmPath, int width, int height)
{
    ...
    qDebug()<<"AVModelPimpl::startedDecoding"<<id<<shmPath;
...
{
    qDebug() << "AVModelPimpl::stoppedDecoding"<<id<<shmPath;
    Q_UNUSED(shmPath)
    removeRenderer(id);
}
...
{
    {
        qDebug()<<"AVModelPimpl::addRenderer"<<id<<shmPath;
        std::lock_guard<std::mutex> lk(renderers_mtx_);
        auto search = renderers_.find(id);
        if (search == renderers_.end()) {
            qDebug()<<"Insert new renderer";
            ...
            if (search->second) {
                qDebug()<<"Update renderer";
                search->second->update(settings.size, shmPath);
            } else {
                qDebug()<<"Reset renderer";
...
{
    qDebug()<<"AVModelPimpl::removeRenderer"<<id;
    {
        std::lock_guard<std::mutex> lk(renderers_mtx_);
        auto search = renderers_.find(id);
        if (search == renderers_.end()) {
