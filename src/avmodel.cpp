...
{
    qDebug() << "AVModel::startPreview";
    VideoManager::instance().startCamera();
    pimpl_->addRenderer(video::PREVIEW_RENDERER_ID, getDeviceSettings(getDefaultDevice()));
...
AVModel::stopPreview()
{
    qDebug() << "AVModel::stopPreview";
    pimpl_->removeRenderer(video::PREVIEW_RENDERER_ID);
    ...
    //        connect(
    //            pimpl_->renderers_[video::PREVIEW_RENDERER_ID].get(),
    //            &api::video::Renderer::stopped,
    //            this,
    //            [this](const QString& id) {
    //                qDebug() << "api::video::Renderer::stopped   " << id;
    //                emit rendererStopped(id);
    //            },
    //            Qt::DirectConnection);
...
{
    qDebug() << Q_FUNC_INFO << QThread::currentThread() << "AVModelPimpl::startedDecoding";
    ...
    //            qDebug() << "startedDecoding w/" << shmPath;
    //            video::Settings settings;
    //            settings.size = res;
    ...
    //                    Qt::UniqueConnection);
    //        } else {
    //            qDebug() << "startedDecoding update w/" << shmPath;
    //            (*search).second->update(res, shmPath);
    //        }
    //    }
    //    connect(renderers_[id].get(), &video::Renderer::started, [this](const QString& id) {
    //        qDebug() << "video::Renderer::started   " << id;
...
{
    qDebug() << "AVModelPimpl::stoppedDecoding 1";
    Q_UNUSED(shmPath)
    removeRenderer(id);
    //    {
    //        std::lock_guard<std::mutex> lk(renderers_mtx_);
    ...
    //        }
    //        if (search->second->getId() != video::PREVIEW_RENDERER_ID) {
...
AVModelPimpl::addRenderer(const QString& id, video::Settings settings, const QString& shmPath)
{
    {
        std::lock_guard<std::mutex> lk(renderers_mtx_);
        auto renderer = std::make_unique<video::Renderer>(id, settings, shmPath, useAVFrame_);
        auto search = renderers_.find(id);
        if (search == renderers_.end()) {
            renderers_.insert(std::make_pair(id, std::move(renderer)));
        } else {
            // search->second->update(settings.size, shmPath);
            search->second.reset(renderer.release());
        }
        connect(renderers_.at(id).get(), &video::Renderer::started, [this](const QString& id) {
            qDebug() << "video::Renderer::started " << id;
            emit linked_.rendererStarted(id);
        });
...
{
    {
        ...
        connect(
            ...
            this,
            [this](const QString& id) {
                qDebug() << "video::Renderer::stopped " << id;
                emit linked_.rendererStopped(id);
                renderers_.erase(id);
            },
            Qt::DirectConnection);
...
{
    qDebug() << "AVModelPimpl::slotFrameUpdated " << id;
