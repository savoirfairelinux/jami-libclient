...
getAtom(xcb_connection_t* c, const std::string& atomName)
{
    ...
    xcb_intern_atom_reply_t* rep = xcb_intern_atom_reply(c, atom_cookie, NULL);
    if (rep) {
...
AVModel::getListWindows() const
{
...
#if defined(Q_OS_UNIX) && !defined(__APPLE__)
    std::unique_ptr<xcb_connection_t, void (*)(xcb_connection_t*)> c(xcb_connect(NULL, NULL),
    ...
    xcb_atom_t atomNetClient = getAtom(c.get(), "_NET_CLIENT_LIST");
    xcb_atom_t atomWMVisibleName = getAtom(c.get(), "_NET_WM_NAME");
    if (!atomNetClient || !atomWMVisibleName) {
        return ret;
    }

    xcb_screen_t* screen = xcb_setup_roots_iterator(xcb_get_setup(c.get())).data;
    ...
    if (replyPropList.get()) {
        ...
        if (valueLegth) {
            xcb_window_t* win = (xcb_window_t*) xcb_get_property_value(replyPropList.get());
