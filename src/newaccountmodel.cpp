...
NewAccountModel::accountVCard(const std::string& accountId, bool compressImage) const
{
    ...
    vCardStr += compressImage ? compressPhoto(accountInfo->second.profileInfo.avatar) : accountInfo->second.profileInfo.avatar;
    vCardStr += vCard::Delimiter::END_LINE_TOKEN;
    vCardStr += vCard::Delimiter::END_TOKEN;
    return vCardStr;
}
std::string NewAccountModel::compressPhoto(std::string img) const
{
    QImage image;
