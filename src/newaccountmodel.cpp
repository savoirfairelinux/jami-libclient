...
NewAccountModel::setAlias(const std::string& accountId, const std::string& alias)
{
    ...
    auto accountProfileId = authority::storage::getOrInsertProfile(accountDb,
        accountInfo.profileInfo.uri, accountId, true,
    to_string(accountInfo.profileInfo.type));
    if (!accountProfileId.empty()) {
        authority::storage::setAliasForProfileId(accountDb, accountProfileId, alias);
    }
...
NewAccountModel::setAvatar(const std::string& accountId, const std::string& avatar)
{
    ...
    to_string(accountInfo.profileInfo.type));
    if (!accountProfileId.empty()) {
        authority::storage::setAvatarForProfileId(accountDb, accountProfileId, avatar);
    }
    emit profileUpdated(accountId);
}
...
{
    const QStringList accountIds = ConfigurationManager::instance().getAccountList();
    auto accountDbs = authority::storage::migrateLegacyDatabaseIfNeeded(accountIds);
...
NewAccountModelPimpl::removeFromAccounts(const std::string& accountId)
{
    ...
    auto& accountInfo = account->second.first;
    auto& accountDb = *(account->second.second);
    authority::storage::removeAccount(accountDb, accountId);
