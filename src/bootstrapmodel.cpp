...
void BootstrapModel::removeRow(int row)
{
   BootstrapModelPrivate::Lines* l =  row < d_ptr->m_lines.size() ?d_ptr->m_lines[row] : nullptr;
   if (l){
       d_ptr->q_ptr->beginRemoveRows(QModelIndex(), row, row);
       d_ptr->m_lines.remove(row);
       delete l;
       d_ptr->save();
       d_ptr->q_ptr->endRemoveRows();
   }
}
