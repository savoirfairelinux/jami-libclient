...
void BootstrapModel::removeRow(const QModelIndex& index)
{
   // Never delete the last row, its a fake empty row
   if (index.row()+1 != rowCount()){
       BootstrapModelPrivate::Lines* toDel = d_ptr->m_lines[index.row()];
       d_ptr->m_lines.remove(index.row());
       emit layoutChanged();
       delete toDel;
   }
}
