...
//Retrieve information from the map and implement the map who contain all newest information
...
void SmartInfoHubPrivate::slotSmartInfo(MapStringMapStringString information){
    SmartInfoHubPrivate::setMapInfo(information);
}
...
float SmartInfoHub::localFps() const{
    if(d_ptr->m_information["local"][FRAMERATE] != NULL)
        return d_ptr->m_information["local"][FRAMERATE].toFloat();
