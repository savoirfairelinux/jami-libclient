    ...
    try {
        ...
        auto cb = std::function<void(QString, QString)>(
            [this, isTemporary, uri, isAudioOnly, &conversation](QString conversationId, QString participantURI) {
                ...
                auto& newConv = isTemporary ? getConversation(conversationId) : conversation;
...
{
    return ConfigurationManager::instance().getConversationInfos(owner.id, conversationId);
}
...
ConversationModel::createConversation(const VectorString& participants, const QString& title)
{
    ...
    if (!title.isEmpty()) {
        auto info = getConversationInfos(convUid);
        info["title"] = title;
...
{
    ConfigurationManager::instance().updateConversationInfo(owner.id, conversationId, info);
}
...
ConversationModel::sendMessage(const QString& uid, const QString& body, const QString& parentId)
{
    try {
        ...
        auto cb = std::function<void(QString, QString)>([this, isTemporary, body, &conversation, parentId, convId](
            ...
            auto& newConv = isTemporary ? pimpl_->getConversation(conversationId) : conversation;
    ...
    try {
        ...
        if (conversation.isSwarm) {
            if (static_cast<DRing::Account::MessageStates>(status) == DRing::Account::MessageStates::DISPLAYED) {
                auto lastDisplayed = conversation.lastDisplayedMessageUid.find(peer);
                if (lastDisplayed == conversation.lastDisplayedMessageUid.end() || conversation.interactions.indexOfMessage(lastDisplayed->second) < conversation.interactions.indexOfMessage(messageId)) {
                    auto lastDisplayedMsg = lastDisplayed->second;
...
ConversationModelPimpl::slotTransferStatusFinished(qulonglong dringId, datatransfer::Info info)
{
    ...
    if (conversationIdx != -1) {
        ...
        if (emitUpdated) {
            ...
            transfIdToConvId.remove(QString::number(dringId));
        }
    }
}
