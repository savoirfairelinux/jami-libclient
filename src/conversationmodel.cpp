...
public:
    ...
    bool usefulDataFromDataTransfer(long long dringId,
                                    ...
                                    QString& convId);
    void awaitingHost(long long dringId, datatransfer::Info info);
    ...
    void startAddingPendingConversation(const QString& contactUri);
...
public Q_SLOTS:
    ...
    bool updateTransferStatus(long long dringId,
    ...
    try {
        ...
        auto cb = std::function<void(QString, QString)>(
            [this, isTemporary, uri, isAudioOnly, &conversation](QString conversationId, QString participantURI) {
                if (isTemporary && (indexOf(conversationId) < 0 || participantURI != conversation.uid) ){
...
ConversationModel::sendMessage(const QString& uid, const QString& body, const QString& parentId)
{
    try {
        ...
        auto cb = std::function<void(QString, QString)>([this, isTemporary, body, &conversation, parentId, convId](
            ...
            if (isTemporary && participantURI != convId) {
                return;
            }
            if (isTemporary && pimpl_->indexOf(conversationId) < 0 ) {
                return;
            }
            if (isTemporary) {
            ...
            if (conversation.isSwarm) {
                ...
                //if (!isTemporary) {
                ConversationManager::instance().loadConversationMessages(owner.id, conversationId,"",1);
                //}
                return;
            }
                                            ...
                                            MapStringString message)
{
    ...
    try {
        ...
        if (msg.type == interaction::Type::DATA_TRANSFER) {
            ...
            auto transferIdInt= std::stoull(transferId.toStdString());
            ...
            msg.status = status == datatransfer::Status::on_connection ? interaction::Status::TRANSFER_AWAITING_HOST : interaction::Status::TRANSFER_CREATED;
            if (msg.status == interaction::Status::TRANSFER_AWAITING_HOST && !interaction::isOutgoing(msg)) {
                conversation.interactions.emplace(msgId, msg);
                conversation.lastMessageUid = msgId;
                                              ...
                                              const QString& conversationId)
{
    ...
    sortConversations();
    emit linked.modelSorted();
    ConversationManager::instance().loadConversationMessages(linked.owner.id,
                                                             conversationId,
                                                                       "",
                                                                       1);
}
...
ConversationModelPimpl::slotContactAdded(const QString& contactUri)
{
    ...
    if (!conv.empty()) {
        ...
        } else if (!pending && indexOf(contactUri) >= 0) {
            //remove temporary pending swarm conversation
            startAddingPendingConversation(contactUri);
        }
        sortConversations();
        emit linked.modelSorted();
    }
...
void
ConversationModelPimpl::startAddingPendingConversation(const QString& contactUri) {
    //TODO notify client
...
ConversationModel::sendFile(const QString& convUid, const QString& path, const QString& filename)
{
    try {
        ...
        auto cb = std::function<void(QString, QString)>(
            ...
            if (isTemporary && (pimpl_->indexOf(conversationId) < 0 || participantURI != peerUri) ){
                qDebug() << "Can't place call: Other participant is not a contact (removed "
                "while placing call ?)";
                return;
            }
                                                   ...
                                                   QString& convId)
{
    ...
    auto iterator = transfIdToConvId.find(QString::number(dringId));
    if (iterator != transfIdToConvId.end()) {
...
void
ConversationModelPimpl::awaitingHost(long long dringId, datatransfer::Info info)
{
    QString interactionId;
    QString convId;
                                       ...
                                       const QString& path)
{
    ...
    auto acceptedFilePath = lrc.getDataTransferModel().accept(interactionId,
                                                              destinationDir + path,
                                                              0);
    qDebug() << "path" << acceptedFilePath;
    qDebug() << "interactionId" << interactionId;
    auto dringId = lrc.getDataTransferModel().getDringIdFromInteractionId(
        interactionId);
...
{
    bool intUpdated;
    updateTransferStatus(dringId, info, interaction::Status::TRANSFER_FINISHED, intUpdated);
}
                                             ...
                                             bool& updated)
{
    ...
    if (conversations[conversationIdx].isSwarm) {
        auto iterator = transfIdToDbIntId.find(QString::number(dringId));
        if (iterator == transfIdToDbIntId.end()) {
            return false;
        }
