...
ConversationModelPimpl::sortConversations()
{
    std::sort(
        ...
        [this](const auto& conversationA, const auto& conversationB)
        {
            ...
            std::string uidA = conversationA.uid;
            std::string uidB = conversationB.uid;
            std::unique_lock<std::mutex> lockConvA(getInteractionsLock(uidA), std::defer_lock);
            std::unique_lock<std::mutex> lockConvB(getInteractionsLock(uidB), std::defer_lock);
