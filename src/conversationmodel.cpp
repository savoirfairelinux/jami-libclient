...
ConversationModel::sendMessage(const std::string& uid, const std::string& body)
{
    ...
    // make a copy of participants list: if current conversation is temporary, it might me destroyed while we are reading it
    ...
    for (const auto& participant: participants) {
        ...
        if (participant.empty()) {
            ...
            // retrieve newly created final conversation
            int contactIndex;
...

    {
        ...
        ret = conversation.interactions.insert(std::pair<uint64_t, interaction::Info>(msgId, msg)).second;
    }
