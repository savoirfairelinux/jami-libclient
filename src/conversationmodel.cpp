...
public:
    ...
    using ConversationReadyCB = std::function<bool(const conversation::Info& convInfo)>;
     ...
     * @warning we could multiple swarm conversations for the same peer. This function will return only one conversation
     */
    ...
    const VectorString& peersForConversation(const conversation::Info& conversation) const;// filter out ourself from conversation partisipants
...
ConversationModel::getConferenceableConversations(const QString& convId, const QString& filter) const
{
    ...
    for (const auto& conv : conversations) {
        ...
        // we want to add only contacts from not group conversation
        auto peers = pimpl_->peersForConversation(conv);
        if (!conv.callId.isEmpty() || !conv.confId.isEmpty() || peers.size() != 1) {
            continue;
        }
        try {
            ...
            accountConv.for_each([this,
                ...
                bool callFilterPredicate = !conv.callId.isEmpty() && conv.callId != currentCallId
                                           && std::find(calls.begin(), calls.end(), conv.callId)
                                                  != calls.end();
                auto peers = pimpl_->peersForConversation(conv);
                if ((!confFilterPredicate && !callFilterPredicate) || peers.size() != 1) {
                    return;
                }
    ...
    return pimpl_->customFilteredConversations.reset(pimpl_->conversations)
        .filter([this, &includeBanned](const conversation::Info& entry) {
            try {
                ...
                auto contactInfo = owner.contactModel->getContact(pimpl_->peersForConversation(entry).front());
...
ConversationModel::getConversationForPeerUri(const QString& uri)
{
    try {
        return std::make_optional(
            pimpl_->getConversation([this, uri](const conversation::Info& conv)
                                        -> bool {
            auto peers = pimpl_->peersForConversation(conv);
            // we could get conversation by peer uri only for one participant conversation
            if (peers.size() != 1) {
                return false;
            }
            return uri == peers.front(); },
                                    true));
    } catch (const std::out_of_range&) {
        return std::nullopt;
    }
...
ConversationModel::makePermanent(const QString& uid)
{
    try {
        ...
        pimpl_->sendContactRequest(pimpl_->peersForConversation(conversation).front());
    } catch (const std::out_of_range& e) {
...
ConversationModel::removeConversation(const QString& uid, bool banned)
{
    ...
    if (pimpl_->peersForConversation(conversation).size() > 1) {
...
ConversationModelPimpl::placeCall(const QString& uid, bool isAudioOnly)
{
    try {
        ...
        auto cb = ([this, isTemporary, uri, isAudioOnly, &conversation](QString conversationId) {
                ...
                auto& newConv = isTemporary ? getConversationForUid(conversationId).get() : conversation;
...
ConversationModel::sendMessage(const QString& uid, const QString& body, const QString& parentId)
{
    try {
        ...
        /* isTemporary, and conversationReady callback used only for non-swarm conversation,
         ...
         Conversations for new contact from search result are NON_SWARM but after receiving conversationReady callback could be updated to ONE_TO_ONE.
         We still use conversationReady callback for swarm conversation with one participant to check if contact is blocked*/
        if (peers.size() != 1) {
        ...
        auto cb = ([this, isTemporary, body, &conversation, parentId, convId](
            ...
            auto& newConv = isTemporary ? pimpl_->getConversationForUid(conversationId).get() : conversation;
    ...
    QString participantURI = "";
    {
        ...
        try {
            ...
            if (messageId != conversation.lastDisplayedMessageUid.end()
                ...
                conversation.lastDisplayedMessageUid.at(pimpl_->peersForConversation(conversation).front())
                    = newDisplayedUid;
            }
    ...
    if (emitUpdated) {
        ...
        if (pimpl_->conversations[conversationIdx].mode != conversation::Mode::NON_SWARM) {
            ...
            if (owner.profileInfo.type != profile::Type::SIP) {
                ...
                .setMessageDisplayed(owner.id,
                                     "jami:"+pimpl_->
                                     peersForConversation(pimpl_->conversations[conversationIdx]).front(),
                                     daemonId,
                                     3);
            }
...
ConversationModel::acceptConversationRequest(const QString& conversationId)
{
    ...
    switch (conversation.mode) {
        ...
        case conversation::Mode::ONE_TO_ONE: {
            ...
            try {
                auto& participantId = pimpl_->peersForConversation(conversation).front();
                auto contact = owner.contactModel->getContact(participantId);
...
ConversationModel::declineConversationRequest(const QString& conversationId, bool banned)
{
    ...
    if (conversation.mode == conversation::Mode::ONE_TO_ONE || conversation.mode == conversation::Mode::NON_SWARM) {
...
ConversationModelPimpl::sort(const conversation::Info& convA, const conversation::Info& convB)
{
    ...
    if (historyA.empty() && historyB.empty()) {
        ...
        return peersForConversation(convA).front() > peersForConversation(convB).front();
    }
    if (historyA.empty())
        return false;
    if (historyB.empty())
                                               ...
                                               const VectorMapStringString& messages)
{
    ...
    try {
        ...
        for (int i = size - 1; i>= 0; --i) {
            ...
            qDebug() << "^^^^ message for" << linked.owner.id << "msgID: " << msgId;
                                            ...
                                            const MapStringString& message)
{
    ...
    try {
        ...
        if (msg.type == interaction::Type::DATA_TRANSFER) {
            // save data transfer interaction to db and assosiate daemon id with interaction id, conversation id and db id
            ...
            msg.body = storage::getCallInteractionString(msg.authorUri, msg.duration);
        } else if (msg.type == interaction::Type::TEXT && msg.authorUri != linked.owner.profileInfo.uri) {
...
void
ConversationModelPimpl::insertSwarmInteraction(const QString& interactionId, const interaction::Info& interaction, conversation::Info& conversation, bool insertAtBegin)
{
                                              ...
                                              const QString& conversationId)
{
    ...
    for (const auto& member : members) {
        ...
        if (member["uri"] != accountURI && shouldRemoveNonSwarmConversation) {
            try {
                                                ...
                                                const QString& conversationId)
{
    ...
    try {
        ...
        // we should create non swarm conversation only for conversation with one peer.
        if (peers.size() != 1) {
            return;
        }
                                 ...
                                 int event)
{
    ...
    // update participants
    auto& conversation = getConversationForUid(conversationId).get();
    const VectorMapStringString& members = ConfigurationManager::instance().getConversationMembers(linked.owner.id, conversationId);
    VectorString uris;
...
ConversationModelPimpl::addSwarmConversation(const QString& convId)
{
    ...
    const VectorMapStringString& members = ConfigurationManager::instance().getConversationMembers(linked.owner.id, convId);
    for (auto& member : members) {
    ...
    conversation.mode = conversation::to_mode(details["mode"].toInt());
    // If conversation has only one peer it is possible that non swarm conversation was created. remove non swarm conversation
    if (peersForConversation(conversation).size() == 1) {
        try {
            auto& participantId = peersForConversation(conversation).front();
...
{
    return getConversation([this, uri](const conversation::Info& conv)
                           -> bool {
        auto peers = peersForConversation(conv);
        if (peers.size() != 1)
            return false;
        return uri == peers.front(); },
                           searchResultIncluded);
...
ConversationModelPimpl::slotIncomingCall(const QString& fromId, const QString& callId)
{
    ...
    if (convIds.empty()) {
        ...
        try {
            ...
            if (contact.profileInfo.type == profile::Type::PENDING && !contact.isBanned && fromId != linked.owner.profileInfo.uri) {
                                           ...
                                           const QString& daemonId)
{
    ...
    if (convIds.empty()) {
        ...
        try {
            ...
            if (contact.profileInfo.type == profile::Type::PENDING && !contact.isBanned && peerId != linked.owner.profileInfo.uri) {
                                                    ...
                                                    int status)
{
    ...
    if (conversationId.isEmpty() || conversationId == linked.owner.profileInfo.uri) {
            ...
            if (it != interactions.end()) {
                ...
                if (messageId != conversation.lastDisplayedMessageUid.end()) {
                    ...
                    if (updateDisplayedUid) {
                        ...
                        conversation.lastDisplayedMessageUid.at(peerId)
                        = it->first;
                    }
                } else {
                    oldDisplayedUid = "";
        ...
        if (updateDisplayedUid) {
            emit linked.displayedInteractionChanged(conversation.uid, peerId, oldDisplayedUid, msgId);
        }
        if (emitUpdated) {
            invalidateModel();
    ...
    try {
        ...
        if (conversation.mode != conversation::Mode::NON_SWARM) {
            if (static_cast<DRing::Account::MessageStates>(status) == DRing::Account::MessageStates::DISPLAYED) {
                if(conversation.lastDisplayedMessageUid.find(peerId) == conversation.lastDisplayedMessageUid.end()) {
                ...
                } else if (conversation.interactions.indexOfMessage(conversation.lastDisplayedMessageUid.find(peerId)->second) < conversation.interactions.indexOfMessage(messageId)) {
                    auto lastDisplayedMsg = conversation.lastDisplayedMessageUid.find(peerId)->second;
                    conversation.lastDisplayedMessageUid[peerId] = messageId;
                    emit linked.displayedInteractionChanged(conversationId, peerId, lastDisplayedMsg, messageId);
                }
            }
        }
    } catch (const std::out_of_range& e) {
...
ConversationModel::setIsComposing(const QString& convUid, bool isComposing)
{
    try {
        ...
        QString to = conversation.mode != conversation::Mode::NON_SWARM ? "swarm:"+convUid
                                        : "jami:"+pimpl_->peersForConversation(conversation).front();
...
ConversationModel::sendFile(const QString& convUid, const QString& path, const QString& filename)
{
    try {
        ...
        /* isTemporary, and conversationReady callback used only for non-swarm conversation,
         because for swarm, conversation already configured at this point.
         Conversations for new contact from search result are NON_SWARM but after receiving conversationReady callback could be updated to ONE_TO_ONE.
         We still use conversationReady callback for one_to_one conversation to check if contact is blocked*/
        if (peers.size() > 1) {
        ...
        auto cb = ([this, isTemporary, peerId, path, filename](QString conversationId) {
            try {
                ...
                auto& conversation = conversationOpt->get();
                auto to = conversation.mode != conversation::Mode::NON_SWARM ? conversationId : peerId;
                
                pimpl_->lrc.getDataTransferModel().sendFile(owner.id, peerId, path, filename);
            } catch(...) {}
        });
...
ConversationModel::cancelTransfer(const QString& convUid, const QString& interactionId)
{
    ...
    if (emitUpdated) {
        // for swarm conversations we need to provide conversation id to accept file, for not swarm conversations we need peer uri
        lrc::api::datatransfer::Info info = {};
        getTransferInfo(convUid, interactionId, info);
...
void
ConversationModel::getTransferInfo(const QString& conversationId, const QString& interactionId, datatransfer::Info& info)
{
    try {
                                                   ...
                                                   QString& conversationId)
{
    ...
    try {
        ...
        conversationId = info.conversationId.isEmpty() ? storage::conversationIdFromInteractionId(db, interactionId) : info.conversationId;
    } catch (const std::out_of_range& e) {
...
ConversationModelPimpl::slotTransferStatusCreated(DataTransferId dringId, datatransfer::Info info)
{
    ...
    if (convIds.empty()) {
        ...
        try {
            ...
            if (contact.profileInfo.type == profile::Type::PENDING && !contact.isBanned && info.peerUri != linked.owner.profileInfo.uri) {
...
void
ConversationModelPimpl::slotTransferStatusAwaitingPeer(DataTransferId dringId, datatransfer::Info info)
{
    bool intUpdated;
                                       ...
                                       const QString& path)
{
    ...
    auto identifier = conversation.mode != conversation::Mode::NON_SWARM ? convUid : peersForConversation(conversation).front();
...
void
ConversationModelPimpl::slotTransferStatusTimeoutExpired(DataTransferId dringId, datatransfer::Info info)
{
    bool intUpdated;
