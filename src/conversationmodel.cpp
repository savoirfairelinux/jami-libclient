...
ConversationModelPimpl::sortConversations()
{
    std::sort(
        ...
        [this](const auto& conversationA, const auto& conversationB)
        {
            ...
            auto mutexA = getInteractionsLock(conversationA.uid);
            auto mutexB = getInteractionsLock(conversationB.uid);
            std::unique_lock<std::mutex> lockConvA(mutexA, std::defer_lock);
            std::unique_lock<std::mutex> lockConvB(mutexB, std::defer_lock);
