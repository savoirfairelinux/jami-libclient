...
public:
    ...
    // Translate id for messages in daemon to messages in database
    // NOTE: We also can store this id in a new column in the "interactions" table
    // because here, if we close the app, we loose linked ids.
    std::map<uint64_t, uint64_t> daemonIdToInteraction;
...
ConversationModel::sendMessage(const std::string& uid, const std::string& body) const
{
    ...
    uint64_t msgId = database::addMessageToConversation(pimpl_->db, accountId, convId, msg);
    // Update conversation
    if (conversation.callId.empty()) {
        // Because the daemon already give an id for the message, we need to store it.
        pimpl_->daemonIdToInteraction.emplace(daemonMsgId, msgId);
    }
                                            ...
                                            const std::string& contactUri)
{
    ...
    for (auto& interaction: conversation.interactions) {
        if (interaction.second.status == interaction::Status::SENDING) {
            // The message has an unknown status because the app was closed.
            // update to Status::UNKNOWN;
            slotUpdateInteractionStatus(linked.owner.id, interaction.first,
                                        contactUri, 0);
        }
    }
...
ConversationModelPimpl::addCallMessage(const std::string& callId, const std::string& body)
{
    ...
    uint64_t msgId = database::addMessageToConversation(db, accountProfileId, conversation.uid, msg);
...
ConversationModelPimpl::addIncomingMessage(const std::string& from, const std::string& body)
{
    ...
    uint64_t msgId = database::addMessageToConversation(db, accountProfileId, conv[0], msg);
                                                    ...
                                                    int status)
{
    ...
    // Update database
    auto msgId = daemonIdToInteraction[id];
