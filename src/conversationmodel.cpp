...
public:
     ...
     * @warning we could  have multiple swarm conversations for the same peer. This function will return one-to-one conversation
     */
    ...
    try {
        return std::make_optional(
            pimpl_->getConversation([this, uri](const conversation::Info& conv)
                                        -> bool {
            auto peers = pimpl_->peersForConversation(conv);
            return uri == peers.front() && conv.isCoreDialog(); },
                                    true));
    } catch (const std::out_of_range&) {
        return std::nullopt;
    }
...
ConversationModel::removeConversation(const QString& uid, bool banned)
{
    ...
    // NOTE: this will also remove the conversation into the database for non-swarm and remove conversation repository for one-to-one.
    owner.contactModel->removeContact(pimpl_->peersForConversation(conversation).first(), banned);
}
                                                ...
                                                const QString& conversationId)
{
    ...
    try {
        ...
        auto contactId = peersForConversation(conversation).first();
                                                  ...
                                                  const bool searchResultIncluded)
{
    return getConversation([this, uri](const conversation::Info& conv)
                           -> bool {
        return uri == peersForConversation(conv).front() && conv.isCoreDialog(); },
                           searchResultIncluded);
}
...
ConversationModelPimpl::indexOfContact(const QString& uri) const
{
    for (unsigned int i = 0; i < conversations.size(); ++i) {
        ...
        if (peers.front() == uri && conversations.at(i).isCoreDialog())
                return i;
    }
    return -1;
}
                                       ...
                                       const QString& path)
{
    ...
    if (emitUpdated) {
        auto peers = peersForConversation(conversations[conversationIdx]);
        if (peers.size() == 1 && conversations[conversationIdx].isCoreDialog()) {
