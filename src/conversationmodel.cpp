...
public:
    ...
    std::mutex& getInteractionsLock(const std::string& convId);
...
ConversationModel::sendMessage(const std::string& uid, const std::string& body)
{
    ...
    {
    	std::lock_guard<std::mutex> lk(pimpl_->getInteractionsLock(convId));
...
ConversationModel::clearHistory(const std::string& uid)
{
    ...
    // Update conversation
    {
        std::lock_guard<std::mutex> lk(pimpl_->getInteractionsLock(uid));
...
ConversationModel::clearAllHistory()
{
    ...
    for (auto& conversation : pimpl_->conversations) {
        {
            std::lock_guard<std::mutex> lk(pimpl_->getInteractionsLock(conversation.uid));
            conversation.interactions.clear();
        }
...
ConversationModelPimpl::initConversations()
{
    ...
    for (auto const& c : linked.owner.contactModel->getAllContacts())
    {
        ...
        std::lock_guard<std::mutex> lk(getInteractionsLock(conversations[convIdx].uid));
...
//            std::unique_lock<std::mutex> lockConvA(getInteractionsLock(conversationA.uid), std::defer_lock);
//            std::unique_lock<std::mutex> lockConvB(getInteractionsLock(conversationB.uid), std::defer_lock);
//            std::lock(lockConvA, lockConvB);

            auto& mtxA = getInteractionsLock(conversationA.uid);
            auto& mtxB = getInteractionsLock(conversationB.uid);
            std::lock(mtxA, mtxB);
                                            ...
                                            const std::string& contactUri)
{
    ...
    {
        std::lock_guard<std::mutex> lk(getInteractionsLock(convId));
    ...
    for (auto& conversation: conversations) {
        if (conversation.callId == callId) {
            ...
            if (newInteraction) {
                ...
                std::lock_guard<std::mutex> lk(getInteractionsLock(conversation.uid));
                                           ...
                                           const std::string& authorProfileId)
{
    ...
    if (conversationIdx == -1) {
        ...
        {
            std::lock_guard<std::mutex> lk(getInteractionsLock(conversations[conversationIdx].uid));
...
ConversationModel::acceptTransfer(const std::string& convUid, uint64_t interactionId, const std::string& path)
{
    ...
    auto conversationIdx = pimpl_->indexOf(convUid);
    if (conversationIdx != -1) {
        std::lock_guard<std::mutex> lk(pimpl_->getInteractionsLock(convUid));
        auto& interactions = pimpl_->conversations[conversationIdx].interactions;
        auto it = interactions.find(interactionId);
        if (it != interactions.end()) {
            it->second.body = path;
...
std::mutex&
ConversationModelPimpl::getInteractionsLock(const std::string& convId)
{
    return interactionsLocks[convId];
}
