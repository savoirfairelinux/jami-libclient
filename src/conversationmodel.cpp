...
ConversationModel::sendMessage(const std::string& uid, const std::string& body) const
{
    ...
    if (conversation.callId.empty()) {
        ...
        database::changeInteractionId(pimpl_->db, msgId, daemonMsgId);
        msgId = daemonMsgId;
    }
                                                    ...
                                                    int status)
{
    ...
    // Update database
    database::updateInteractionStatus(db, id, newStatus);
    ...
    if (!conv.empty()) {
        ...
        if (conversationIdx != -1) {
            ...
            if (interactions.find(id) != interactions.end()) {
                interactions[id].status = newStatus;
                emit linked.interactionStatusUpdated(conv[0], id, interactions[id]);
            }
        }
    }
}
