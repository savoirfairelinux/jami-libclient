        ...
        std::for_each(interactions.begin(),
                      ...
                      [&](decltype(*interactions.begin())& it) {
                          if (!it.second.isRead) {
                              emitUpdated = true;
                              it.second.isRead = true;
                              if (owner.profileInfo.type != profile::Type::SIP && !pimpl_->conversations[conversationIdx].isSwarm)
...
ConversationModelPimpl::initConversations()
{
    ...
    VectorString swarm = ConfigurationManager::instance().getConversations(linked.owner.id);
                                 ...
                                 int event)
{
    ...
    // update participants
    auto conversation = getConversation(conversationId);
    auto members = ConfigurationManager::instance().getConversationMembers(linked.owner.id, conversationId);
    VectorString uris;
...
ConversationModelPimpl::removePendingConversation(const QString& contactUri) {
    ...
    *connection = connect(&this->linked,
                          &ConversationModel::conversationReady,
                          [cb, connection, contactUri](QString convId, QString participantURI) {
...
{
    VectorString uris;
    auto members = ConfigurationManager::instance().getConversationMembers(linked.owner.id, convId);
    auto accountURI = linked.owner.profileInfo.uri;
    for (auto& member : members) {
        if (member["uri"] != accountURI) {
            uris.append(member["uri"]);
...
{
    auto convUid = ConfigurationManager::instance().startConversation(linked.owner.id);
    ConfigurationManager::instance().addConversationMember(linked.owner.id, convUid, participantUri);
