                                               ...
                                               VectorMapStringString messages)
{
    ...
    try {
        ...
        //for (auto& message : messages) {
            ...
            int it = conversation.interactions.indexOfMsg(msg.parentId);
            if (it >= 0) {
                it = it + 1;
                conversation.interactions.insert(it, qMakePair(msgId, msg));
            } else {
                conversation.interactions.insert(std::make_pair(msgId, msg));
                conversation.roots[msgId] = msg.parentId;
            }
            if (conversation.roots.values().contains(msgId)) {
                auto msgIds = conversation.roots.keys(msgId);
                conversation.interactions.moveMsg(msgIds, msgId);
                for (auto msg : msgIds) {
                    conversation.roots.remove(msg);
                }
            }
                                            ...
                                            MapStringString message)
{
    ...
    try {
        ...
        if (it >= 0) {
            it = it + 1;
            auto parent = conversation.interactions.at(msg.parentId);
            qDebug() << "parentId" << msg.parentId;
            qDebug() << "parent body" << parent.body;
            conversation.interactions.insert(it, qMakePair(msgId, msg));
        } else {
            conversation.interactions.insert(std::make_pair(msgId, msg));
            conversation.roots[msgId] = msg.parentId;
