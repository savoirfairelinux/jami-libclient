...
ConversationModelPimpl::placeCall(const std::string& uid, bool isAudioOnly)
{
    ...
    if (isTemporary) {
        QObject::connect(&this->linked, &ConversationModel::conversationReady, [cb] { cb(); });
    }
    else {
        cb();
    }
...
ConversationModel::sendMessage(const std::string& uid, const std::string& body)
{
    ...
    if (isTemporary) {
        QObject::connect(this, &ConversationModel::conversationReady, [cb] { cb(); });
    }
    else {
        cb();
    }
