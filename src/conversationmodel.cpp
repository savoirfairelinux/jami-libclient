...
public:
    ...
    // for each contact we must have one non-swarm conversation or one active one-to-one conversation. Where active means peer did not leave the conversation.
    ...
    try {
        ...
        auto cb = ([this, isTemporary, body, &conversation, parentId, convId](
            ...
            if (not newConv.callId.isEmpty()
                ...
                daemonMsgId = owner.contactModel->sendDhtMessage(peers.front(),
                                                                 body);
            }
                                              ...
                                              const QString& conversationId)
{
    ...
    for (const auto& member : members) {
        // this check should be removed once all usage of participants replaced by peersForConversation. We should have ourself in participants list
    ...
    invalidateModel();
    // we use conversationReady callback only for conversation with one participant. We could use participants.front()
    auto& peers = peersForConversation(conversation);
    if (peers.size() == 1) {
