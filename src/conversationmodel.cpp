...
ConversationModelPimpl::sortConversations()
{
    std::sort(
        ...
        [this](const auto& conversationA, const auto& conversationB)
        {
            ...
            std::unique_lock<std::mutex> lockConvA(getInteractionsLock("test"), std::defer_lock);
            std::unique_lock<std::mutex> lockConvB(getInteractionsLock("test"), std::defer_lock);
