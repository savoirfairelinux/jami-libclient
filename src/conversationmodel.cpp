...
ConversationModelPimpl::placeCall(const QString& uid, bool isAudioOnly)
{
    try {
        ...
        auto cb = std::function<void(QString, QString)>(
            [this, isTemporary, uri, isAudioOnly, &conversation](QString convId, QString participantURI) {
                int contactIndex;
...
ConversationModel::sendFile(const QString& convUid, const QString& path, const QString& filename)
{
    try {
        ...
        auto cb = std::function<void(QString, QString)>(
            [this, isTemporary, peerUri, path, filename](QString convId, QString participantURI) {
                int contactIndex;
...
ConversationModel::cancelTransfer(const QString& convUid, const QString& interactionId)
{
    ...
    if (emitUpdated) {
        ...
        pimpl_->lrc.getDataTransferModel().cancel(interactionId.toInt());
        pimpl_->dirtyConversations = {true, true};
                                       ...
                                       const QString& path)
{
    ...
    if (!dir.exists())
        dir.mkpath(".");
    auto acceptedFilePath = lrc.getDataTransferModel().accept(interactionId.toInt(),
                                                              destinationDir + path,
                                                              0);
