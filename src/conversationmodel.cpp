    ...
    try {
        ...
        if (conversation.isSwarm) {
            if (static_cast<DRing::Account::MessageStates>(status) == DRing::Account::MessageStates::DISPLAYED) {
                ...
                bool findLastDisplayed = lastDisplayed == conversation.lastDisplayedMessageUid.end();
                if (findLastDisplayed || conversation.interactions.indexOfMessage(lastDisplayed->second) < conversation.interactions.indexOfMessage(messageId)) {
                    auto lastDisplayedMsg = findLastDisplayed ? lastDisplayed->second : "";
