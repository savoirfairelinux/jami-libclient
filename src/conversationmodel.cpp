                                            ...
                                            const int size)
{
    ...
    if (conversation.allConversationLoaded) {
        return -1;
    }
        if (conversation.interactions.empty()) {
    ...
    auto lastMsgId = conversation.interactions.front().first;
    return ConversationManager::instance().loadConversationMessages(owner.id,
                                                                    conversationId,
                                                                    lastMsgId,
                                                                    size);
    ...
    try {
        ...
        //for (auto& message : messages) {
            ...
            if (message["type"] == "initial") {
                conversation.allConversationLoaded = true;
            }
                                            ...
                                            MapStringString message)
{
    ...
    try {
        ...
        if (msg.type == interaction::Type::DATA_TRANSFER) {
            ...
            msg.status = (status == datatransfer::Status::on_connection && !interaction::isOutgoing(msg)) ? interaction::Status::TRANSFER_AWAITING_HOST : interaction::Status::TRANSFER_CREATED;
