...
ConversationModel::setIsComposing(const QString& convUid, bool isComposing)
{
    try {
        ...
        QString to = conversation.mode != conversation::Mode::NON_SWARM ? "swarm:" + convUid : "jami:" + pimpl_->peersForConversation(conversation).front();
                                       ...
                                       const QString& path)
{
    ...
    auto acceptedFilePath = linked.owner.dataTransferModel->accept(linked.owner.id,
                                                              ...
                                                              destinationDir + path,
                                                              0);
    auto dringId = linked.owner.getDataTransferModel().getDringIdFromInteractionId(interactionId);
