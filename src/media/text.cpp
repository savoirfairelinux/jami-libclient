...
void IMConversationManagerPrivate::accountMessageStatusChanged(uint64_t id, const QString& to, DRing::Account::MessageStates status)
{
    if (auto cm = PhoneDirectoryModel::instance().getNumber(to)) {
        auto txtRecording = cm->textRecording();
        txtRecording->d_ptr->accountMessageStatusChanged(id, status);
    }
}
