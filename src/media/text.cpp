...
void IMConversationManagerPrivate::accountMessageStatusChanged(uint64_t id, const QString& to, const QString& status)
{
    if (auto cm = PhoneDirectoryModel::instance().getNumber(to)) {
        auto txtRecording = cm->textRecording();
        txtRecording->accountMessageStatusChanged(id, status);
    }
}
