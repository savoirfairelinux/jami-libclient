...
{
    auto node = m_pPendingMessages.value(id, nullptr);
    if (node) {
        node->m_pMessage->deliveryStatus = static_cast<int>(status);
    ...
    for (const Serializable::Peers* p : t->d_ptr->m_lAssociatedPeers) {
        ...
        for (const Serializable::Group* g : p->groups) {
            for (Serializable::Message* m : g->messages) {
                ...
                if (m->id)
                    t->d_ptr->m_pPendingMessages[m->id] = n;
            }
        }
...
QHash<int,QByteArray> InstantMessagingModel::roleNames() const
{
   ...
   if (!initRoles) {
      ...
      roles.insert((int)Media::TextRecording::Role::IsSending           , "isSending"           );
      roles.insert((int)Media::TextRecording::Role::HasSucceeded        , "hasSucceeded"        );
   }
   return roles;
}
...
QVariant InstantMessagingModel::data( const QModelIndex& idx, int role) const
{
   if (idx.column() == 0) {
      ...
      switch (role) {
         ...
         case (int)Media::TextRecording::Role::IsSending            :
            return n->m_pMessage->deliveryStatus == static_cast<int>(DRing::Account::MessageStates::SENDING);
         case (int)Media::TextRecording::Role::HasSucceeded         :
            return n->m_pMessage->deliveryStatus == static_cast<int>(DRing::Account::MessageStates::SENT);
         default:
            break;
      }
   }
