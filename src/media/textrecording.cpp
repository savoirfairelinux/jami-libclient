        ...
        for (const Serializable::Group* g : p->groups) {
            for (Serializable::Message* m : g->messages) {
                ...
                if (m->id) {
                    ...
                    if (auto status =
                            static_cast<DRing::Account::MessageStates>(configurationManager.getMessageStatus(m->id))
                            != DRing::Account::MessageStates::UNKNOWN)
...
void Serializable::Message::read (const QJsonObject &json)
{
   ...
   deliveryStatus = static_cast<Media::TextRecording::Status>(json["deliveryStatus"].toVariant().toInt());
