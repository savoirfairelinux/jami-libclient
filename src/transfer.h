...
public:
   ...
   Q_PROPERTY(DRing::DataTransferError  error           READ error)
   Q_PROPERTY(ContactMethod*            contactMethod   READ contactMethod  WRITE setContactMethod)
   Q_PROPERTY(DRing::FileTransferStatus status          READ status         WRITE setStatus)
   ...
   DRing::DataTransferError error() const;
   DRing::FileTransferStatus status() const;
   int size()                   const;
   int progress() const;
   ContactMethod* contactMethod() const;

   //Setter
   void setStatus       (DRing::FileTransferStatus status);
...
Q_SIGNALS:
   void changed();
   void statusChanged(const QString&, DRing::FileTransferStatus);
