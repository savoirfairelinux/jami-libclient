...
///Set the private key
void Account::setTlsPrivateKey(const QUrl& path)
{
    auto cert = tlsCertificate();
    if (!cert)
        return;

    d_ptr->setAccountProperty(DRing::Account::ConfProperties::TLS::PRIVATE_KEY_FILE, cert?path.path():QString());
...
void Account::setTlsCaListCertificate(Certificate* cert)
{
   ...
   d_ptr->setAccountProperty(DRing::Account::ConfProperties::TLS::CA_LIST_FILE, cert?cert->path().toString():QString());
   d_ptr->regenSecurityValidation();
...
void Account::setTlsCertificate(Certificate* cert)
{
   ...
   d_ptr->setAccountProperty(DRing::Account::ConfProperties::TLS::CERTIFICATE_FILE, cert?cert->path().toString():QString());
   d_ptr->regenSecurityValidation();
}
