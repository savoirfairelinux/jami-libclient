...
{
    return ConfigurationManager::instance().registerName(id(), password, name);
}

//Lookup a name
bool Account::lookupName(const QString& name) const
{
    return ConfigurationManager::instance().lookupName(id(), name);
}

//Lookup an address
bool Account::lookupAddress(const QString& address) const
{
    return ConfigurationManager::instance().lookupAddress(id(), address);
}
