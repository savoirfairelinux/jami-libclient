...
public:
    ...
    ContainerView(const Container& container)
    {
        data_ = std::make_optional(std::ref(container));
        auto& dataSource = std::remove_const_t<Container&>(data());
    ...
    ContainerView& reset(const Container& container)
    {
        data_ = std::make_optional(std::ref(container));
        auto& dataSource = std::remove_const_t<Container&>(data());
        view_.assign(dataSource.begin(), dataSource.end());
...
private:
    ...
    const Container& data() { return data_.value().get(); }
