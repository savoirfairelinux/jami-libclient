...
private:
    ...
    {
        using type = Container<NewType, typename rebind<Args, NewType>::type...>;
    };

    using value_type = std::remove_reference_t<decltype(std::declval<Container>().front())>;
    using const_reference = const value_type&;
    using view_type = typename rebind<Container, std::reference_wrapper<value_type>>::type;
...
public:
    ...
    ContainerView(Container& container)
        ...
        , view_(container.begin(), container.end()) {};
    ContainerView(const ContainerView& other)
        : data_(std::nullopt)
        , view_(other().begin(), other().end()) {};
    ContainerView& operator=(const ContainerView& other)
    {
        data_ = other.data_;
        view_ = other.view_;
        dirty_ = other.dirty_;
    ...
    ContainerView& operator=(ContainerView&& other) = delete;
    ...
    {
        data_ = std::make_optional(std::ref(container));
        view_.assign(container.begin(), container.end());
        invalidate();
        return *this;
    }

    // Alternately, reset the view to another view and disregard underlying data.
    ContainerView& reset(ContainerView& other)
    {
        data_ = std::nullopt;
        view_.assign(other().begin(), other().end());
        invalidate();
        return *this;
    }
    ...
    // re-filtering ?? should maybe observe underlying data
...
private:
    ...
    Container& data() { return data_.value().get(); }
