   ...
   ConfigurationManagerInterface()
   {
      ...
      confHandlers = {
         ...
         exportable_callback<DRing::ConfigurationSignal::AccountMessageStatus>(
               [this] (uint64_t id, const std::string& to, const std::string& status) {
               QTimer::singleShot(0, [this, id, to, status] {
                     Q_EMIT this->accountMessageStatus(id, QString(to.c_str()), QString(status.c_str()));
               });
         }),
...
public Q_SLOTS: // METHODS
   ...
   QString getMessageStatus(uint64_t id)
   {
       return QString::fromStdString(DRing::getMessageStatus(id));
   }
...
Q_SIGNALS: // SIGNALS
   ...
   void accountMessageStatus(const uint64_t id, const QString& to, const QString& status);
