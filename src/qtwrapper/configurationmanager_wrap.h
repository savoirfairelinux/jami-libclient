      ...
      confHandlers = {
         ...
         exportable_callback<DRing::ConfigurationSignal::AccountMessageStatus>(
               ...
               QTimer::singleShot(0, [this, accountID, id, to, status] {
                     Q_EMIT this->accountMessageStatus(QString(accountID.c_str()), id, QString(to.c_str()), static_cast<DRing::Account::MessageStates>(status));
               });
         }),
...
Q_SIGNALS: // SIGNALS
   ...
   void accountMessageStatus(const QString& accountId, const uint64_t id, const QString& to, DRing::Account::MessageStates status);
