   ...
   ConfigurationManagerInterface()
   {
      ...
      confHandlers = {
         ...
         exportable_callback<DRing::ConfigurationSignal::AccountMessageStatus>(
               [this] (uint64_t id, const std::string& to, int status) {
               QTimer::singleShot(0, [this, id, to, status] {
                     Q_EMIT this->accountMessageStatus(id, QString(to.c_str()), static_cast<DRing::Account::MessageStates>(status));
               });
         }),
...
Q_SIGNALS: // SIGNALS
   ...
   void accountMessageStatus(const uint64_t id, const QString& to, DRing::Account::MessageStates status);
