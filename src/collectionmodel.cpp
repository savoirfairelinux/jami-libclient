      ...
      if (item->collection) {
         switch(role) {
            ...
            case Qt::CheckStateRole: {
               try {
                  Interfaces::itemModelStateSerializer(); //TODO do better than that
...
bool CollectionModel::setData (const QModelIndex& idx, const QVariant &value, int role )
{
   ...
   if (role == Qt::CheckStateRole && idx.column() == 0) {
      ...
      if (item && item->collection) {
         ...
         try {
            Interfaces::itemModelStateSerializer().setChecked(item->collection,value==Qt::Checked);
...
bool CollectionModel::save()
{
   try {
      ...
      foreach(CollectionModelPrivate::ProxyItem* top, d_ptr->m_lTopLevelBackends) {
         ...
         if (current) {
            check = Interfaces::itemModelStateSerializer().isChecked(current);
      ...
      return Interfaces::itemModelStateSerializer().save();
   } catch (...) {
...
{
   try {
      return Interfaces::itemModelStateSerializer().load();
   } catch (...) {
      return false;
   }
}
