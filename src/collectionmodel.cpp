      ...
      if (item->collection) {
         switch(role) {
            ...
            case Qt::CheckStateRole: {
               if (getDelegateManager()->getItemModelStateSerializationDelegate()) //TODO do better than that
...
bool CollectionModel::setData (const QModelIndex& idx, const QVariant &value, int role )
{
   ...
   if (role == Qt::CheckStateRole && idx.column() == 0) {
      ...
      if (item && item->collection) {
         ...
         if (getDelegateManager()->getItemModelStateSerializationDelegate()) {
            getDelegateManager()->getItemModelStateSerializationDelegate()->setChecked(item->collection,value==Qt::Checked);
...
bool CollectionModel::save()
{
   if (getDelegateManager()->getItemModelStateSerializationDelegate()) {
      ...
      foreach(CollectionModelPrivate::ProxyItem* top, d_ptr->m_lTopLevelBackends) {
         ...
         if (current) {
            check = getDelegateManager()->getItemModelStateSerializationDelegate()->isChecked(current);
      ...
      return getDelegateManager()->getItemModelStateSerializationDelegate()->save();
   }
   return false;
}
...
{
   if (getDelegateManager()->getItemModelStateSerializationDelegate()) {
      return getDelegateManager()->getItemModelStateSerializationDelegate()->load();
   }
   return false;
}
