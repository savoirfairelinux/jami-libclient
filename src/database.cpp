...
Database::migrateSchemaFromVersion1()
{
    ...
    auto insertIsAccount = "ALTER TABLE profiles ADD COLUMN is_account TEXT";
    ...
    */
    ...
    for (auto accountId : accountIds) {
        ...
        for (auto contact_info : contacts_vector) {
            ...
            updateOrInsertAccountIDForUri(contactURI.toStdString(), accountId.toStdString());
        }
...
void
Database::updateOrInsertAccountIDForUri(const std::string& contactURI,
                              const std::string& accountId)
{
    // check if the URI exsist and if the row already updated
    auto rows = select("account_id", "profiles","uri=:uri",
                     {{":uri", contactURI}})
                     .payloads;
    if (rows.empty()) {
        return;
    }
    // if we have just one entry for URI and account ID not set, update this row with an account id.
    if (rows.size() == 1 && rows[0].empty()) {
      ...
      // if profile already associated with one account twe need to add another row
        auto aliasQuery = select("alias", "profiles","uri=:uri",
                                 {{":uri", contactURI}})
                                 .payloads;
        auto photoQuery = select("photo", "profiles","uri=:uri",
                                 {{":uri", contactURI}})
                                 .payloads;
        auto typeQuery = select("type", "profiles","uri=:uri",
                                {{":uri", contactURI}})
                                .payloads;
        auto statusQuery = select("status", "profiles","uri=:uri",
                                  {{":uri", contactURI}})
                                  .payloads;
        auto alias = aliasQuery.empty() ? "" : aliasQuery[0];
        auto avatar = photoQuery.empty() ? "" : photoQuery[0];
        auto type = typeQuery.empty() ? "INVALID" : typeQuery[0];
        auto status = statusQuery.empty() ? "TRUSTED" : statusQuery[0];
        ...
        for (const auto& account: accs) {
            ...
            if (select("id", "profiles","uri=:uri", {{":uri", uri.toStdString()}}).payloads.empty()) {
                insertInto("profiles",
                           ...
                           {":status", "TRUSTED"}, {":account_id", accountId.toStdString()},
                           {":is_account", "true"}});
            }
        }
    }
