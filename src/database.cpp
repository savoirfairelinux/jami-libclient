...
: QObject()
{
    ...
    try
    {
        ...
        db_.setDatabaseName(QStandardPaths::writableLocation(QStandardPaths::DataLocation) + "/" + name_.c_str());
...
Database::createTables() const
{
    ...
    return storeVersion(version_);
}

bool
Database::storeVersion(const std::string& version) const
{
    QSqlQuery query;
