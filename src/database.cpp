    ...
    : QObject(), connectionName_(QString::fromStdString(name))
{
    ...
    auto databaseFile = QFileInfo(getPath() + "/" + connectionName_ + ".db");
...
Database::createTables()
{
    ...
    auto indexConversations = "CREATE UNIQUE INDEX `idx_conversations_uri` ON `conversations` (`participant`)";
    ...
    auto indexInteractions = "CREATE UNIQUE INDEX `idx_interactions_uri` ON `interactions` (`author`)";
    ...
    // add indexing
    if (not query.exec(indexConversations)) {
        throw QueryError(query);
    }

    if (not query.exec(indexInteractions)) {
        throw QueryError(query);
    }
