...
{
   return getDelegateManager()->getPixmapManipulationDelegate()->collectionIcon(this,PixmapManipulationDelegate::CollectionIconHint::CONTACT);
}

bool FallbackPersonCollection::isEnabled() const
{
   /* if delegate exists, check if collectin is enabled, else assume it is */
   if (auto delegate = getDelegateManager()->getItemModelStateSerializationDelegate())
