...
void VideoRendererManagerPrivate::startedDecoding(const QString& id, const QString& shmPath, int width, int height)
{
   ...
   if (!m_hRenderers.contains(rid)) {
      ...
      DBus::VideoManager::instance().registerSinkTarget(id, [this, id, width, height] (const unsigned char* frame, int w, int h) {
         static_cast<Video::DirectRenderer*>(m_hRenderers[id.toLatin1()])->onNewFrame(
            QByteArray::fromRawData(reinterpret_cast<const char *>(frame),w*h), w, h
         );
      });
