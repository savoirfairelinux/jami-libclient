   ...
   if (index.isValid()) {
      switch (role) {
         ...
         case Qt::DecorationRole:
            return Delegates::getPixmapManipulationDelegate()->contactSortingCategoryIcon(static_cast<CategorizedContactModel::SortedProxy::Categories>(index.row()));
      }
   }
   return QVariant();
}
...
QVariant HistorySortingCategoryModel::data( const QModelIndex& index, int role) const
{
   if (index.isValid()) {
      switch(role) {
         ...
         case Qt::DecorationRole:
            return Delegates::getPixmapManipulationDelegate()->historySortingCategoryIcon(static_cast<CategorizedHistoryModel::SortedProxy::Categories>(index.row()));
      }
   }
