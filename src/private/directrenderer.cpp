...
public:
    ...
    mutable QByteArray rendererFrame_;
...
DRing::SinkTarget::FrameBufferPtr Video::DirectRendererPrivate::requestFrameBuffer(std::size_t bytes)
{
    ...
    if (not daemonFramePtr_)
        daemonFramePtr_.reset(new DRing::SinkTarget::FrameBuffer);
    daemonFramePtr_.resize(bytes);
    daemonFramePtr_->ptr = daemonFramePtr_.data();
