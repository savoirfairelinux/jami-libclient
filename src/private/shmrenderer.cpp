...
bool ShmRendererPrivate::getNewFrame(bool wait)
{
   ...
   if (not q_ptr->Video::Renderer::d_ptr->m_pFrameBuffer)
       q_ptr->Video::Renderer::d_ptr->m_pFrameBuffer.reset(new DRing::FrameBuffer);

   q_ptr->Video::Renderer::d_ptr->m_pFrameBuffer->ptr     = m_pShmArea->data + m_pShmArea->readOffset;
   q_ptr->Video::Renderer::d_ptr->m_pFrameBuffer->ptrSize = m_pShmArea->frameSize;
...
DRing::SinkTarget::FrameBufferPtr ShmRenderer::currentFrame() const
{
    if (not isRendering())
        return {};
    ...
    return std::move(Video::Renderer::d_ptr->m_pFrameBuffer);
}
