...
: m_Parsed(false)
, m_HeaderType(URI::SchemeType::NONE)
, linked(uri)
, m_hasChevrons(false)
, m_HasAt(false)
, m_ProtocolHint(URI::ProtocolHint::SIP_OTHER)
, m_HintParsed(false)
, m_IsHNParsed(false)
, m_Port(-1)
, m_Transport(URI::Transport::NOT_SET)
{
}

URI::URI() : QString(), pimpl_(new URIPimpl(this))
{
}

URI::URI(const QString& other) : URI()
{
    pimpl_->m_Stripped = URIPimpl::strip(other.simplified().remove(' ').remove('<').remove('>'), pimpl_->m_HeaderType);
    (*static_cast<QString*>(this)) = pimpl_->m_Stripped;
}

URI::URI(const URI& o) : URI()
{
    pimpl_->commonCopyConstructor(o);
}
...
void URIPimpl::commonCopyConstructor(const URI& o)
{
    m_Parsed = o.pimpl_->m_Parsed;
    m_Scheme = o.pimpl_->m_Scheme;
    m_HintParsed = o.pimpl_->m_HintParsed;
    m_ExtHostname = o.pimpl_->m_ExtHostname;
    m_HasAt = o.pimpl_->m_HasAt;
    m_ProtocolHint = o.pimpl_->m_ProtocolHint;
    m_HeaderType = o.pimpl_->m_HeaderType;
    m_Userinfo = o.pimpl_->m_Userinfo;
    m_Stripped = o.pimpl_->m_Stripped;
    m_IsHNParsed = o.pimpl_->m_IsHNParsed;
    m_Port = o.pimpl_->m_Port;
    m_Transport = o.pimpl_->m_Transport;
    m_Tag = o.pimpl_->m_Tag;

    (*static_cast<QString*>(linked)) = o.pimpl_->m_Stripped;
}
 ...
 */
QString URIPimpl::strip(const QString& uri, URI::SchemeType& scheme)
{
    if (uri.isEmpty())
        return {};

    m_Scheme = std::regex_match(uri.toStdString, std::regex("[a-zA-Z][a-zA-Z0-9+.-]*:")).c_str();

    if (m_Scheme == URIPimpl::schemeNames[URI::SchemeType::SIP]) {
        scheme = URI::SchemeType::SIP;
    } else if (m_Scheme == URIPimpl::schemeNames[URI::SchemeType::RING]) {
        scheme = URI::SchemeType::RING;
    } else if (m_Scheme == URIPimpl::schemeNames[URI::SchemeType::SIPS]) {
        scheme = URI::SchemeType::SIPS;
    } else {
        scheme = URI::SchemeType::UNRECOGNIZED;
    }

    return uri.mid(m_Scheme.size(), uri.size());
}
...
void URI::setSchemeType(SchemeType t)
{
    ...
    pimpl_->mScheme = URIPimpl::schemeNames[t]; 
}
