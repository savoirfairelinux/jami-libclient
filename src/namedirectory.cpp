...
void NameDirectoryPrivate::slotNameRegistrationEnded(const QString& accountId, int status, const QString& name)
{
   ...
   emit q_ptr->nameRegistrationEnded(accountId, static_cast<NameDirectory::RegisterNameStatus>(status), name);

   Account* a = AccountModel::instance().getById(accountId.toLatin1());
   if (a) {
       emit a->nameRegistrationEnded(static_cast<NameDirectory::RegisterNameStatus>(status), name);
   }
   else {
...
void NameDirectoryPrivate::slotRegisteredNameFound(const QString& accountId, int status, const QString& address, const QString& name)
{
    ...
    emit q_ptr->registeredNameFound(accountId, static_cast<NameDirectory::LookupStatus>(status), address, name);

    Account* a = AccountModel::instance().getById(accountId.toLatin1());
    if (a) {
        emit a->registeredNameFound(static_cast<NameDirectory::LookupStatus>(status), address, name);
    }
    else {
...
//Register a name
bool NameDirectory::registerName(const QString& accountId, const QString& password, const QString& name) const
{
    return ConfigurationManager::instance().registerName(accountId, password, name);
}

//Lookup a name
bool NameDirectory::lookupName(const QString& accountId, const QString& nameServiceURL, const QString& name) const
{
    return ConfigurationManager::instance().lookupName(accountId, nameServiceURL, name);
}

//Lookup an address
bool NameDirectory::lookupAddress(const QString& accountId, const QString& nameServiceURL, const QString& address) const
{
