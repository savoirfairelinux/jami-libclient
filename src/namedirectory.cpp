...
void NameDirectory::addrLookup(const QString& addr, LookupCallback cb)
{
    ...
    if (not cacheRes.isEmpty()) {
        cb(cacheRes, BlockChainResponse::found);
        return;
    }
        ...
        if (reply->error() == QNetworkReply::NoError) {
            ...
            if (v >= 200 && v < 300) // Success
            {
                ...
                if (not name.isEmpty()) {
                    ...
                    cb(name, BlockChainResponse::found);
                } else {
                    cb("", BlockChainResponse::notFound);
                }
            }
        } else {
            cb("", BlockChainResponse::error);
        }
    });
}
        ...
        if (reply->error() == QNetworkReply::NoError) {
            ...
            if (v >= 200 && v < 300) // Success
            {
                ...
                if (not name.isEmpty()) {
                    ...
                    cb(addr, BlockChainResponse::found);
                }
            }
            cb("", BlockChainResponse::notFound);
        } else {
            QString error("Network error : "+reply->errorString());
            cb(error, BlockChainResponse::error);
        }
    });
}
