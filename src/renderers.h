...
public:
    ...
    lrc::NewShmRenderer& getRenderer(const std::string& callId);
    lrc::NewShmRenderer* getPreviewRenderer() const;
    ...
    lrc::NewShmRenderer& from(const std::string& rendererId); // [jn] renderer id can be call id or local
...
private: // [jn] pimpl me that
    ...
    std::map<std::string, std::unique_ptr<lrc::NewShmRenderer>> renderers_;
};
