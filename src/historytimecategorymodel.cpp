...
HistoryTimeCategoryModel::HistoryConst HistoryTimeCategoryModel::timeToHistoryConst(const time_t time3)
{
   ...
   uint elapsed_seconds = currentTime - time;
   uint elapsed_days    = (elapsed_seconds        ) / day          ;
   uint elapsed_weeks   = (elapsed_seconds - week ) / week         ;
   uint elapsed_months  = (elapsed_seconds - month) / (30.4f * day);
