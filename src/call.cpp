...
void Call::UpdateCallOutgoingMedia() const
{
   auto list = media(Media::Media::Type::VIDEO, Media::Media::Direction::OUT);
   const auto& details = CallPrivate::getCallDetailsCommon(d_ptr->m_DringId);
   ...
   if(list.size() <=0){
       ...
       d_ptr->CallPrivate::mediaFactory<Media::Video>(direction);
   }
   printf("\x1b[31m DEBUG:\x1b[0m""      |function:%s file:%s line:%d\n",__FUNCTION__,__FILE__,__LINE__);
    //todo info update
   list = media(Media::Media::Type::VIDEO, Media::Media::Direction::OUT);
