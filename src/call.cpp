...
void CallPrivate::prepareOutgoingMedia()
{
   ...
   if(video_source.length() <= 0 && list.isEmpty()){ //Means there is no video, and there never was. Nothing to do.
       return;
   }

   if(list.isEmpty() ){//update data
...
Call* CallPrivate::buildCall(const QString& callId, Call::Direction callDirection, Call::State startState)
{
    ...
    auto call = std::unique_ptr<Call, decltype(deleteCall)&>( new Call(startState, peerName, nb, acc),
                                                             deleteCall );
    call->d_ptr->prepareOutgoingMedia();
