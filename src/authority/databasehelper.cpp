                        ...
                        const std::string& peer_uri)
{
    ...
    const auto vCard = VCardUtils::toHashMap(vcard);
    ...
    if (interactionsResult.nbrOfCols == nCols) {
        ...
        for (decltype(payloads.size()) i = 0; i < payloads.size(); i += nCols) {
            ...
            if (!extra_data_str.isEmpty()) {
                duration = readJSONValue(JSONFromString(extra_data_str),
            ...
            auto msg = api::interaction::Info({
                    ...
                    duration.empty() ? 0 : std::stoi(duration),
                   ...
                   const std::string& daemonId)
{
    ...
    if (msgAlreadyExists.empty()) {
        ...
        db.update("interactions",
                  ...
                  {{":body", msg.body}, {":duration", std::to_string(msg.duration)} },
                  "id=:id", { {":id", id} });
        return std::stoi(id);
    }
                               ...
                               const std::time_t& duration)
{
    ...
    auto extra_data_str = getInteractionExtraDataById(db, idStr);
        ...
        try {
            ...
            while (interactionIt != allInteractions.payloads.end()) {
                ...
                bool is_read = (status == "READ"
                    || type == "DATA_TRANSFER"
                    || type == "CALL"
                    || migratedMsg.first == msgFlag::IS_CONTACT_ADDED
                    || migratedMsg.first == msgFlag::IS_INVITATION_ACCEPTED);
