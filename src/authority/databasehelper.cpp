...
beginConversationWithPeer(Database& db, const std::string& peer_uri, const bool isOutgoing)
{
    ...
    auto status = isOutgoing ? api::interaction::Status::SUCCESS : api::interaction::Status::UNKNOWN;
    api::interaction::Info msg{
        peer_uri, {}, std::time(nullptr), 0, api::interaction::Type::CONTACT, status, false
    };
                              ...
                              migrationCallback& didMigrateCb)
{
    ...
    std::thread migrateThread(
        [&appPath, &accountIds, &dbs, &legacyDb, &didMigrateCb] {
            ...
            if (didMigrateCb) didMigrateCb();
        });

    // willMigrateCb should block and be unblocked by didMigrateCb
    if (willMigrateCb) willMigrateCb();
    migrateThread.join();
