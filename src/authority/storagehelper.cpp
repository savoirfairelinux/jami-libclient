...
getAccountAvatar(const std::string& accountId)
{
    ...
    const auto photo = (vCard.find(vCard::Property::TYPE_PNG) == vCard.end()) ?
        vCard[vCard::Property::TYPE_JPEG] : vCard[vCard::Property::TYPE_PNG];
                 ...
                 std::shared_ptr<Database> legacyDb)
{
    ...
    for (const auto& profileId : profileIds) {
        ...
        if (uriScheme == URI::SchemeType::NONE) {
            ...
            profileInfo.type = uriScheme == URI::SchemeType::RING ?
                profile::Type::RING :
                profile::Type::SIP;
        }
                              ...
                              MigrationCb& didMigrateCb)
{
    ...
    // first check if the legacy database still exists
    auto legacyDBFilePath = appPath + "ring.db";
    {
        QFile file(legacyDBFilePath);
        if (!file.open(QIODevice::ReadOnly)) {
