...
class NewShmRendererPimpl : public QObject
{
    Q_OBJECT
public:
    NewShmRendererPimpl(const NewShmRenderer& linked, const std::string& shmPath, bool startedDecoding);
    ~NewShmRendererPimpl();
    bool startShm();
    void stopShm();
    bool remapShm();
    bool getNewFrame();
    int sharedMem_; // file descriptor
    ...
    void shmUnlock();
    bool isRendering_;
    const NewShmRenderer& linked;
};

NewShmRendererPimpl::NewShmRendererPimpl(const NewShmRenderer& linked, const std::string& shmPath, bool startedDecoding)
: shmPath_(shmPath)
, sharedMem_(-1)
, isRendering_(startedDecoding)
, linked(linked)
{
    //~ qDebug() << "X1: " << isRendering_;
    //~ if (isRendering_)
        //~ linked.startRendering();
}
...
NewShmRenderer::NewShmRenderer(const std::string& shmPath, int width, int height, bool startedDecoding)
: lrc::BaseRender::BaseRender(width, height)
, pimpl_(std::make_unique<NewShmRendererPimpl>(*this, shmPath, startedDecoding))
{
    qDebug() << "X2: " << pimpl_->isRendering_;
}
...
bool
NewShmRenderer::startRendering() const
{
    //~ QMutexLocker locker {mutex()};
