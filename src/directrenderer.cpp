...
public:
    ...
    FrameBufferPtr pullFrameBuffer(std::size_t bytes);
    void pushFrameBuffer(FrameBufferPtr buf);
    ...
    mutable FrameBufferBasePtr daemonFramePtr_;
...
Video::DirectRendererPrivate::pullFrameBuffer(std::size_t bytes)
{
    ...
    return std::move(std::make_unique<FrameBuffer>(bytes));
}
...
void
Video::DirectRendererPrivate::pushFrameBuffer(FrameBufferPtr buf)
{
