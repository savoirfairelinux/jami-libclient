...
{
    PluginManager::instance().saveStateLoadedPlugins(rootPath, false);
...
bool PluginModel::loadPlugin(const QString& path)
{
    ...
    PluginManager::instance().saveStateLoadedPlugins(path, status);
    return status;
}

bool PluginModel::unloadPlugin(const QString& path)
{
    PluginManager::instance().saveStateLoadedPlugins(path, false);
...
void PluginModel::toggleCallMediaHandler(const QString& id)
{
    MapStringString toggleInfo = PluginManager::instance().isToggledMediaHandler();
...
MapStringString PluginModel::isToggledMediaHandler()
{
    return PluginManager::instance().isToggledMediaHandler();
}
