...
//    qDebug() << "MessageListModel::insertMessage" << index << interactions_.size()
//             << message.second.body;
}

iterator
api::MessageListModel::insertMessage(iterator it, item_t& message)
{
    auto index = std::distance(begin(), it);
    Q_EMIT beginInsertRows(QModelIndex(), index, index);
    auto insertion = interactions_.insert(it, message);
    Q_EMIT endInsertRows();
//    qDebug() << "MessageListModel::insertMessage2" << index << interactions_.size()
//             << message.second.body;
    return insertion;
}

void
api::MessageListModel::removeMessage(int index, iterator it)
{
...
void
api::MessageListModel::moveMessage(int from, int to)
{
...
MessageListModel::dataForItem(item_t item, int indexRow, int role) const
{
    ...
    case Role::HyperlinkInformation:
        return QVariant(item.second.hyperlinkPreviewInformation);
    case Role::Index:
        return QVariant(getIndexOfMessage(item.first));
    case Role::LinkifyStatus:
        return QVariant(static_cast<int> (item.second.linkifyStatus));
    default:
        break;
    }
}
...
int
MessageListModel::getIndexOfMessage(QString messageId) const
{
...
void
MessageListModel::addHyperlinkInfo(QString messageId, QVariantMap hyperlinkInformation)
{
    ...
    interactions_[index].second.hyperlinkPreviewInformation = hyperlinkInformation;
    Q_EMIT dataChanged(modelIndex, modelIndex, {Role::HyperlinkInformation});
}

void
MessageListModel::addLinkifiedMessage(QString messageId, QString linkifiedMessage, bool urlPresent)
{
    int index = getIndexOfMessage(messageId);
    if (index == -1) {
        return;
    ...
    QModelIndex modelIndex = QAbstractListModel::index(index, 0);
    interactions_[index].second.body = linkifiedMessage;
    interactions_[index].second.linkifyStatus = urlPresent ? interaction::LinkifyStatus::URL_PRESENT : interaction::LinkifyStatus::NO_URL;
    Q_EMIT dataChanged(modelIndex, modelIndex, {Role::MessageBody, Role::LinkifyStatus});
}
