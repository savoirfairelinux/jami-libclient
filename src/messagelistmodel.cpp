...
void
MessageListModel::linkifyMessage(const QString& messageId, const QString& linkified, bool hasUrl)
{
    int index = getIndexOfMessage(messageId);
    if (index == -1) {
        return;
    ...
    interactions_[index].second.linkifyStatus = hasUrl ? interaction::LinkifyStatus::URL_PRESENT
                                                       : interaction::LinkifyStatus::NO_URL;
    Q_EMIT dataChanged(modelIndex, modelIndex, {Role::Body, Role::LinkifyStatus});
}
