...
MessageListModel::emplace(const QString& msgId, interaction::Info message, bool beginning)
{
    ...
    auto inserted = insertMessage(interactions_.size(), iter, qMakePair(msgId, message));
    return qMakePair(inserted, true);
}
...
MessageListModel::operator[](const QString& messageId)
{
    ...
    insertMessage(interactions_.size(), interactions_.end(), qMakePair(messageId, newMessage));
...
MessageListModel::insert(int index, QPair<QString, interaction::Info> message)
{
    ...
    if (index >= size()) {
        insertMessage(interactions_.size(), interactions_.end(), message);
        return qMakePair(interactions_.end(), true);
    }
    insertMessage(index, message);
    return qMakePair(begin() + index, true);
}
...
iterator
api::MessageListModel::insertMessage(int index, iterator it, item_t& message)
{
...
{
    qDebug() << "MessageListModel::removeMessage" << index;
...
{
    qDebug() << "MessageListModel::moveMessage" << from << to;
