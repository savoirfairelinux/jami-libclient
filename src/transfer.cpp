...
Transfer::Transfer(Account* a, const QString& id, bool isOutgoing) : d_ptr(new TransferPrivate(this))
{
    d_ptr->m_pAccount = a;
    d_ptr->m_Id = id;
    d_ptr->m_pOutgoing = isOutgoing;
}
...
bool Transfer::isOutgoing() const
{
    return d_ptr->m_pOutgoing;
}

bool Transfer::isPending()  const
{
    return !d_ptr->m_pOutgoing && d_ptr->m_Status == DRing::DataTransferCode::CODE_NOTIFYING;
}
