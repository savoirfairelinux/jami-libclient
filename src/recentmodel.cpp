...
PeopleProxy::filterAcceptsRow(int source_row, const QModelIndex & source_parent) const
{
    ...
    if (!source_parent.isValid() && filterRegExp().isEmpty()) {
        ...
        if (type == Ring::ObjectType::ContactMethod) {
            ...
            // it doesn't make sense to show a contactMethod in conversation if there was no conversation. But we need
            // this for now, as long as contact are not working well.
            // change this by : first tryin to add an account (use chosen account if possible) to each contact method
            // otherwise just don't show the contactmethod in conver
            if (not cm->account())
                return cm;
