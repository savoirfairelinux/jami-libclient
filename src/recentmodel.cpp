...
RecentViewNode::childNode(Call *call) const
{
    ...
    auto itEnd = m_lChildren.end();
    auto it = std::find_if (m_lChildren.begin(),
...
RecentModelPrivate::insertCallNode(RecentViewNode *parent, Call* call)
{
    ...
    q_ptr->beginInsertRows(q_ptr->index(parent->m_Index,0), callNode->m_Index, callNode->m_Index);
    ...
    if (callNode->m_Index == 2) {
        ...
        auto firstChild = q_ptr->index(0, 0, q_ptr->index(parent->m_Index, 0));
...
{
    qDebug() << "\t\t***  removing call" << call;
    // make sure the call is removed from the call bucket or the model
    if (m_lCallBucket.contains(call))
        m_lCallBucket.removeAll(call);
...
RecentModelPrivate::slotCallAdded(Call* call, Call* parent)
{
   ...
   if (auto parent = parentNode(call)) {
       ...
       // no Person or CM associated with the call yet, add it to the call bucket
...
RecentModelPrivate::updateCall(Call* call)
{
    ...
    // otherwise check if it needs to be transfered from the call bucket to the model
    switch(call->state()) {
