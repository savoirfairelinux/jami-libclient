    ...
    if (!source_parent.isValid()) {
        ...
        switch (type) {
            case Ring::ObjectType::Person:
            {
                ...
                auto passesFilter = std::any_of(std::begin(person_numbers), std::end(person_numbers),
                [&](const ContactMethod* cm)
                {
                    // only proceed if there is no account set yet, or if it matches the chosen account
                    if ( !cm->account() or (cm->account() == chosen_account)) {
                         ...
                         */
                        auto containsRegEx =
                            person->formattedName().contains(filterRegExp()) or
            ...
            case Ring::ObjectType::ContactMethod:
            {
                ...
                // only proceed if there is no account set yet, or if it matches the chosen account
                if ( !cm->account() or (cm->account() == chosen_account)) {
                    /* we need to check any identifier of the ContactMethod.
                     * note: QString::contains() will return true for an empty param string
                     */
                    passesFilter =
                        cm->uri().full().contains(filterRegExp()) or
