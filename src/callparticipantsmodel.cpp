...
CallParticipants::CallParticipants(const VectorMapStringString& infos, const QString& callId)
{
    callID = callId;
    update(infos);
}
...
{
    //qDebug() << "getparticipants" << participants_;
...
CallParticipants::removeParticipant(int index)
{
    ...
    emit participantRemoved(callID, idx_);
}

void
CallParticipants::addParticipant(const ParticipantInfos& participant)
{
    auto peerId = participant.uri;
    auto it = participants_.find(peerId);
    if (it == participants_.end()) {
        participants_.insert(participants_.begin() + idx_, peerId, participant);
        emit participantAdded(callID, idx_);
    } else {
        ...
        if (participant.uri == it->uri && participant.sinkId == it->sinkId &&
            ...
            return;
        (*it) = participant;
        emit participantUpdated(callID, idx_);
    }
}
    ...
    for (const auto& candidate : infos) {
        ...
        if (candidate["w"].toInt() != 0
            ...
            if (it == candidates_.end()) {
                candidates_.insert(peerId, ParticipantInfos(candidate, callID));
            } else {
                (*it) = ParticipantInfos(candidate, callID);
            }
        }
    }
}
