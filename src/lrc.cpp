...
#include "../../daemon/src/dring/call_const.h"
...
Lrc::activeCalls()
{
    ...
    for (const auto &call : callLists) {
        auto callDetails = CallManager::instance().getCallDetails(call);
        if(isUnFinished(callDetails[DRing::Call::Details::CALL_STATE]))
