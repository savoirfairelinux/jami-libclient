...
NewCallModel::getParticipantsInfos(const QString& callId)
{
    if (pimpl_->participantsModel.find(callId) == pimpl_->participantsModel.end()) {
        ...
        pimpl_->participantsModel.emplace(callId, std::make_shared<CallParticipants>(infos, callId));
    }
...
NewCallModelPimpl::initCallFromDaemon()
{
    ...
    for (const auto& callId : callList) {
        ...
        if (accountId == linked.owner.id) {
            ...
            participantsModel.emplace(callId, std::make_shared<CallParticipants>(infos, callId));
                                                ...
                                                const VectorMapStringString& infos)
{
    ...
    if (!it->second->isConference && !infos.isEmpty()) {
        it->second->isConference = true;
        emit linked.onParticipantsChanged(confId);
    } else if (it->second->isConference && infos.isEmpty()) {
        it->second->isConference = false;
        emit linked.onParticipantsChanged(confId);
    }
    ...
    if (participantsModel.find(confId) == participantsModel.end())
        participantsModel.emplace(confId, std::make_shared<CallParticipants>(infos, confId));
    else
...
NewCallModelPimpl::slotConferenceCreated(const QString& confId)
{
    ...
    participantsModel[confId] = std::make_shared<CallParticipants>(infos, confId);
