...
{
    // [jn] cette fonction doit etre appellé depuis le sendinteraction de conversation
    ...
    // l'ajout du interaction dans la db doit se faire depuis conversation
}
    ...
    if (it != calls.end()) {
        ...
        if (it_2 != vcardsChunks.end()) {
            ...
            if ( not std::any_of(vcardsChunks[from].begin(), vcardsChunks[from].end(),
                ...
                vcardsChunks.erase(from); // Transfer is finish, ze don't want to reuse this entry.
            }
        } else {
