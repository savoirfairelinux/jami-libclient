...
, callbacksHandler(callbacksHandler)
{
    ...
    for (const auto& callId: callList)
    {
        ...
        if (accountId == linked.owner.id) {
            ...
            auto diff = static_cast<long>(system_now) - std::stol(details["TIMESTAMP_START"].toStdString());
            ...
            // HACK: mute/unmute the video to restart private/videorenderermanager.
    ...
    // Get active conferences from daemon
    ...
    for (const auto& callId: callList)
    {
        ...
        foreach(const auto& call, callList) {
            ...
            auto now = std::chrono::steady_clock::now();
            auto system_now = std::chrono::system_clock::to_time_t(std::chrono::system_clock::now());
            auto diff = static_cast<long>(system_now) - std::stol(callDetails["TIMESTAMP_START"].toStdString());
