...
{
public:
    NewCallModelPimpl(const NewCallModel& linked, const CallbacksHandler& callbacksHandler, const BehaviorController& behaviorController);
    ~NewCallModelPimpl();
...
NewCallModel::NewCallModel(const account::Info& owner, const CallbacksHandler& callbacksHandler, const BehaviorController& behaviorController)
    : QObject(nullptr)
    , owner(owner)
...
NewCallModelPimpl::slotCallStateChanged(const QString& callId, const QString& state, int code)
{
    ...
    if (status == call::Status::ENDED && !call::isTerminating(call->status)) {
        ...
        emit behaviorController.callStatusChanged(callId);
    }
