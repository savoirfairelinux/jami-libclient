...
/**
 * ensureUid ensures an unique Id. By checking the current uid first and generate a random one if not unique or empty.
 */
void
Person::ensureUid()
{
    ...
    if (d_ptr->m_Uid.isEmpty()) {
        ...
        } while (PersonModel::instance().getPersonByUid(d_ptr->m_Uid) != this);
        return;
    }
