...
template<typename T>
bool SqlManager::createTable() const {

   QSqlQuery createQuery;
   if (std::is_same<T, Call>::value) {
      createQuery.prepare(QString("CREATE TABLE IF NOT EXISTS call_history (%1 TEXT UNIQUE, %2 TEXT, %3 TEXT, %4 TEXT, %5 TEXT, %6 TEXT, %7 TEXT, %8 INTEGER, %9 INTEGER, %10 TEXT, %11 TEXT, %12 TEXT)")
...
template<typename T>
bool SqlManager::deleteTable() const {
   if (db.isOpen()) {
      QSqlQuery query(QString("DELETE TABLE IF EXISTS %1").arg(getTableName<T>()));
      return query.exec();
   }
   return false;
}
