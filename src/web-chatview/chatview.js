...
function updateFileInteraction(message_div, message_object, forceTypeToFile = false) {
    ...
    if (isAudio(message_text) && message_delivery_status === "finished" && displayLinksEnabled && !forceTypeToFile) {
        ...
        if (use_qt) {
            ...
            var internal_mes_wrapper_final = document.createElement("div")
            internal_mes_wrapper_final.setAttribute("class","internal_mes_wrapper")
            message_div.insertBefore(internal_mes_wrapper_final, message_div.querySelector(".menu_interaction"))
        }
