...
function sendMessage() {
    ...
    var img_src
    var img_src_container = new Array()
    while ((img_src = imgSrcExtract.exec(data_to_send)) !== null) {
        img_src_container.push(img_src[1])
    }

    var file_src
    var file_src_container = new Array()
    while ((file_src = fileSrcExtract.exec(data_to_send)) !== null) {
        file_src_container.push(file_src[1])
    }

    if (use_qt) {
        img_src_container.forEach(function(image) {
            window.jsbridge.sendImage(image)
        })
        file_src_container.forEach(function (file) {
            window.jsbridge.sendFile(file)
        })
    } else {
        img_src_container.forEach(function (image) {
            window.prompt("SEND_IMG:" + image)
        })
        file_src_container.forEach(function (file) {
            window.prompt("SEND_FILE:" + file)
        })
    }
...
function addAudio_Path(path) {
    ...
    var html = "<div class=\"file_wrapper\" data-path=\"" + path + "\">" +
        ...
        "</div>" +
        "<span id=\"audio_player_timer_text_" + id + "\" class=\"audio_player_timer_text\" type=\"text\"></span>" +
...
function playPause(id, btn) {
    ...
    currentAudio.onended = function () {
        ...
        document.getElementById("audio_player_timer_text_" + id).innerHTML = "00:00/" + getFormattedTimeDisplay(currentAudio.duration)
...
function removeAudioWrapperAndDeleteAudioFile(e,path) {
    if (use_qt) {
        ...
        window.prompt("DELETE_SELECTED_FILE:" + path)
    }
    remove(e)
}
...
function addAudioController() {
    ...
    if (!document.getElementById("audio_record_controller")) {
        // Add the controller to the first position of the send container
        var containerContentHtml = sendContainer.innerHTML
        containerContentHtml = html + containerContentHtml
        sendContainer.innerHTML = containerContentHtml
    }
    updateMesPos()
    startTimer()
}
...
function startRecordingSection() {
    ...
    if (document.getElementById("audio_record_controller")) {
        // if it's already in recording, fisnish currenct record and start a new recorder
        finishAudioRecord()
    }
    if (use_qt) {
        window.jsbridge.startRecordAudio()
    } else {
        window.prompt("START_RECORD_AUDIO")
    }
...
function finishAudioRecord() {
    if (use_qt) {
        ...
        window.prompt("FINISH_RECORD_AUDIO")
    }
    if (document.getElementById("audio_record_controller")) {
...
function stopAndDeleteAudioRecord(e) {
    if (use_qt) {
        ...
        window.prompt("STOP_AND_DELETE_RECORD_AUDIO")
    }
    remove(e)
    stopTimer()
}
