...
function addOrUpdateMessage(message_object, new_message, insert_after = true, messages_div) {
    ...
    if (new_message) {
        ...
        if (message_type === "call" || message_type === "contact") {
            ...
            const selector = use_qt ? ".timestamp_cell" : ".internal_mes_wrapper"
            message_div.querySelector(selector).appendChild(date_elt)
            if (message_direction === "out")
                message_div.querySelector(selector).setAttribute("class", "timestamp_cell_out")
        }
