...
{
   if (auto call = q_ptr->getCall(callId)) {
      auto media = call->firstMedia<Media::Audio>(Media::Media::Direction::OUT);
      if (state)
         media->Media::d_ptr->muteConfirmed();
      else
         media->Media::d_ptr->unmuteConfirmed();
   }
}
...
{
   if (auto call = q_ptr->getCall(callId)) {
      auto media = call->firstMedia<Media::Video>(Media::Media::Direction::OUT);
      if (state)
         media->Media::d_ptr->muteConfirmed();
      else
         media->Media::d_ptr->unmuteConfirmed();
