...
#include "delegates/contactmethodselectordelegate.h"
...
bool CallModel::dropMimeData(const QMimeData* mimedata, Qt::DropAction action, int row, int column, const QModelIndex& parentIdx )
{
   ...
   if (mimedata->hasFormat(RingMimes::CALLID)) {
      ...
      if (getDelegateManager()->getContactMethodSelectorDelegate()) {
         const ContactMethod* number = getDelegateManager()->getContactMethodSelectorDelegate()->getNumber(
