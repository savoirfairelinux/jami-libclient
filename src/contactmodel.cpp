...
public:
     ...
     * @param status
     */
    void newBuddySubscription(const std::string& uri, bool status);
    /**
...
, callbacksHandler(callbacksHandler)
{
    ...
    connect(&callbacksHandler, &CallbacksHandler::newBuddySubscription,
            this, &ContactModelPimpl::newBuddySubscription);
...
ContactModelPimpl::fillsWithSIPContacts()
{
    ...
    for (const auto& c : conversationsForAccount) {
        ...
        for (const auto& participant: otherParticipants) {
            ...
            {
                std::lock_guard<std::mutex> lk(this->linked.contactsMtx_);
...
void
ContactModelPimpl::newBuddySubscription(const std::string& contactUri, bool status)
{
    std::lock_guard<std::mutex> lk(this->linked.contactsMtx_);
