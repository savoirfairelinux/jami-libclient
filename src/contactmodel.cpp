...
ContactModel::searchContact(const std::string& query)
{
    ...
    if (uriScheme == URI::SchemeType::SIP && owner.profileInfo.type == profile::Type::SIP) {
        ...
        pimpl_->updateTemporaryMessage(tr("Bad URI scheme"), uri.full().toStdString());
    }
}
...
ContactModelPimpl::searchRingContact(const std::string& query)
{
    ...
    updateTemporaryMessage(tr("Searching…"), query);
                                           ...
                                           const std::string& registeredName)
{
    ...
    if (status == 0 /* SUCCESS */) {
        ...
        switch (status) {
        case 1 /* INVALID */:
            updateTemporaryMessage(tr("Invalid ringID"), registeredName);
            break;
        case 2 /* NOT FOUND */:
            updateTemporaryMessage(tr("Not found"), registeredName);
            break;
        case 3 /* ERROR */:
