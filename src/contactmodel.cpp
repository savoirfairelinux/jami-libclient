...
public:
    ...
    void updateTemporaryMessage(const std::string& error, const std::string& uri);
...
ContactModel::searchContact(const std::string& query)
{
    ...
    if (uriScheme == URI::SchemeType::SIP && owner.profileInfo.type == profile::Type::SIP) {
        ...
        pimpl_->updateTemporaryMessage("Bad URI scheme", uri.full().toStdString());
    }
}

void
ContactModelPimpl::updateTemporaryMessage(const std::string& error, const std::string& uri)
{
    std::lock_guard<std::mutex> lk(contactsMtx_);
    auto& temporaryContact = contacts[""];
    temporaryContact.profileInfo.alias = error;
...
ContactModelPimpl::searchRingContact(const std::string& query)
{
    ...
    updateTemporaryMessage("Searching…", query);
                                           ...
                                           const std::string& registeredName)
{
    ...
    if (status == 0 /* SUCCESS */) {
        ...
        switch (status) {
        case 1 /* INVALID */:
            updateTemporaryMessage("Invalid ringID", registeredName);
            break;
        case 2 /* NOT FOUND */:
            updateTemporaryMessage("Not found", registeredName);
            break;
        case 3 /* ERROR */:
            updateTemporaryMessage("Couldn't lookup…", registeredName);
            break;
        }
    }
