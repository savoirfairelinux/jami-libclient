...
ContactModel::ContactModel(ConversationModel& parent, const Database& db, const account::Info& info)
: parent_(parent)
, db_(db)
, owner(info)
, QObject()
...
ContactModel::removeContact(const std::string& uri)
{
    ...
    ConfigurationManager::instance().removeContact(QString(owner.id.c_str()), QString(uri.c_str()), false);
}
...
ContactModel::sendMessage(const std::string& uri, const std::string& body) const
{
    ...
    unsigned int id = ConfigurationManager::instance().sendTextMessage(QString(owner.id.c_str()), uri.c_str(), payloads);
...
{
    if (owner.id != accountID.toStdString()) return;
    // TODO disconnect signal from LRC
    if (contacts_.find(uri.toStdString()) != contacts_.end()) {
        db_.clearHistory(owner.id, uri.toStdString(), true);
