...
ContactModel::addContact(contact::Info contactInfo)
{
    ...
    database::getOrInsertProfile(pimpl_->db, profile.uri, owner.id, false, to_string(owner.profileInfo.type),
                                              ...
                                              const std::string& payload)
{
    ...
    auto emitTrust = false;
    {
        ...
        if (contacts.find(contactUri) == contacts.end()) {
            ...
            emitTrust = true;
            database::getOrInsertProfile(db, contactUri, accountId, false, profile::to_string(profile::Type::RING),
            alias.toStdString(),
                                         photo.toStdString()
                                         );
        }
