...
ContactModel::addContact(const std::string& contactUri)
{
    ...
    if(owner.profile.type == contact::Type::SIP) {
        ...
        if(contactFound != pimpl_->contacts.end() && contactFound->second.type == contact::Type::PENDING) {
            ...
            // Test if the daemon considers this uri as a contact
            ...
            if (details.empty()) {
                ...
                // HACK: The daemon says this uri is a contact but it's not returned by getAllContacts
                ...
                pimpl_->contacts.emplace(contactUri, contact);
                auto returnFromDb = pimpl_->db.select("photo, uri, alias",
                                              "profiles",
                                              "uri=:uri",
                                              {{":uri", contactUri}});
                if (returnFromDb.payloads.empty()) {
                    // If profile not in db, create profile.
                    pimpl_->db.insertInto("profiles",
                                 {{":uri", "uri"}, {":alias", "alias"},
                                              ...
                                              const std::string& payload)
{
    ...
    if (contacts.find(contactUri) == contacts.end()) {
        ...
        contacts.emplace(contactUri, contact);
        ...
        if (returnFromDb.payloads.empty()) {
            ...
            db.update("profiles",
                      ...
                      {{":alias", contact.alias}, {":photo", contact.avatar}},
                      "uri=:uri", {{":uri", contact.uri}});
        }
        emit linked.modelUpdated();
    }
}
