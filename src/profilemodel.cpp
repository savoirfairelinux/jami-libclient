...
bool ProfileEditor::remove(const Person* item)
{
   ...
   if (QFile::remove(path(item))) {
      ...
      for (;nodeIdx < m_lProfiles.size(); nodeIdx++) {
         if (m_lProfiles[nodeIdx]->contact == item) {
            isFound = true;
            break;
         }
      }
...
QModelIndex ProfileModel::mapFromSource(const QModelIndex& idx) const
{
   ...
   Q_ASSERT(accNode && accNode->parent && accNode->type == Node::Type::ACCOUNT);
...
Node* ProfileModelPrivate::insertProfile(Person* p)
{
   ...
   m_pProfileBackend->m_pEditor->m_hProfileByPerson[p] = pro; //FIXME
