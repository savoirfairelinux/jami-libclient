...
LRCMedia::TextRecording* ContactMethod::textRecording() const
{
    if (!d_ptr->m_pTextRecording) {
        d_ptr->m_pTextRecording = LRCMedia::RecordingModel::instance().createTextRecording(this);
    }
...
void ContactMethodPrivate::setTextRecording(LRCMedia::TextRecording* r)
{
   m_pTextRecording = r;
}
...
bool ContactMethod::sendOfflineTextMessage(const QMap<QString,QString>& payloads)
{
   ...
   auto id = ConfigurationManager::instance().sendTextMessage(selectedAccount->id()
                                                    ...
                                                    ,payloads);
   txtRecording->d_ptr->insertNewMessage(payloads, this, LRCMedia::Media::Direction::OUT, id);
   return true;
}
