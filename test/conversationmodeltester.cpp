...
ConversationModelTester::testFilterBannedContact()
{
    ...
    WaitForSignalHelper(*accInfo_.contactModel,
        SIGNAL(modelUpdated())).wait(1000);
    CPPUNIT_ASSERT_EQUAL((std::deque<lrc::api::conversation::Info>::size_type) 2, accInfo_.conversationModel->allFilteredConversations().size());
}
...
ConversationModelTester::testSendMessageToBannedContact()
{
    ...
    for (const auto& conversation: conversations) {
        if (std::find(conversation.participants.begin(), conversation.participants.end(), contactInfo.profileInfo.uri) != conversation.participants.end()) {
            ...
            // Ban bannedContact
            accInfo_.conversationModel->removeConversation(conversation.uid, true);
            auto contactBanned = WaitForSignalHelper(*(accInfo_.conversationModel),
                SIGNAL(lrc::api::ConversationModel::filterChanged())).wait(1000);
            CPPUNIT_ASSERT_EQUAL(contactBanned, true);
            contactInfo = accInfo_.contactModel->getContact(newContactUri);
            ...
            CPPUNIT_ASSERT_EQUAL(0, (int)conversation.interactions.size());
            break;
        }
    }
