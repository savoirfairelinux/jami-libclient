...
ContactModelTester::testReceivesContactPresenceUpdate()
{
    ...
    const auto& accInfo = lrc_->getAccountModel().getAccountInfo(accountId);
    int nbContactsAtBegin = accInfo.contactModel->getAllContacts().size();
    auto accountIdBob = daemon_->getAccountId("Bob");
    const auto& accInfoBob = lrc_->getAccountModel().getAccountInfo(accountIdBob);

    // "Ada" should be in "Bob" contacts.
    CPPUNIT_ASSERT_NO_THROW(accInfo.contactModel->getContact(accInfoBob.profileInfo.uri));
    auto idx = 0;
    while (!accInfo.contactModel->getContact(accInfoBob.profileInfo.uri).isPresent && idx < 10) {
    ...
    CPPUNIT_ASSERT_EQUAL(accInfo.contactModel->getContact(accInfoBob.profileInfo.uri).isPresent, true);
...
ContactModelTester::testRmContact()
{
    ...
    auto removeContactSigsCaught = WaitForSignalHelper([&]() {
            accInfo.contactModel->removeContact(accInfoBob.profileInfo.uri);
        })
    ...
    CPPUNIT_ASSERT_THROW(accInfo.contactModel->getContact(accInfoBob.profileInfo.uri), std::out_of_range);
}
