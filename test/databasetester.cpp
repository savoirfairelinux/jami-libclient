...
DatabaseTester::testUnread()
{
    ...
    database_->clearHistory(accountId, contactUri);
    CPPUNIT_ASSERT_EQUAL(database_->numberOfUnreads(accountId, contactUri), 0);
    database_->addMessage(accountId, msg);
    CPPUNIT_ASSERT_EQUAL(database_->numberOfUnreads(accountId, contactUri), 1);
    auto messages = database_->getHistory(accountId, contactUri);
    CPPUNIT_ASSERT_EQUAL(1, messages.size());
    database_->setMessageRead(messages.begin()->front);
    CPPUNIT_ASSERT_EQUAL(database_->numberOfUnreads(accountId, contactUri), 0);
}
...
bool
DatabaseTester::findMessage(const lrc::message::Info& msg,
                            ...
                            const std::string& contactUri) const
{
    ...
    for (const auto item : messages) {
        ...
        if (message.uid == msg.uid && msg.body == message.body
