...
private:
    QMap<QString, VectorMapStringString> availableContacts_;
...
public Q_SLOTS: // METHODS
    ...
    VectorMapStringString getContacts(const QString &accountId)
    {
        ...
        if (accountId.indexOf("ring") == -1) return contacts;
        for (auto c = 0; c < availableContacts_.size() ; ++c) {
            auto contact = std::map<std::string, std::string>();
            contact.insert(std::pair<std::string, std::string>("id", availableContacts_.at(c)));
