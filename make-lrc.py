...
def generate(force, qtver, sdk, arch):
    # check build dir
    ...
    if os.path.exists(build_dir):
        if not force:
            ...
            return
        purge_dir(build_dir)
    print('Generating lrc with Qt: ' + qtver + ' ' + arch)
    this_dir = os.path.dirname(os.path.realpath(__file__))
...
def build(qtver, arch):
    print('Building lrc with Qt: ' + qtver + ' ' + arch)
    vs_env_vars = {}
...
def main():
    ...
    if parsed_args.purge:
        ...
        generate(parsed_args.force, parsed_args.qtver,
                 parsed_args.sdk, parsed_args.arch)

    if parsed_args.build:
        build(parsed_args.qtver, parsed_args.arch)
